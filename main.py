...
while inGame:
    ...
    if room_number == "7":
        if room_number not in rooms:
            print("""Egy fejszét találsz a padlón. Legalább most van valami a kezedben.""")
            items.append("balta")
            rooms.append(room_number)
        else:
            print("A szoba üres.")
    ...
    if room_number == "13":
        ...
        if "balta" in items:
            print("Felhasználod a baltát amit a földön találtál egy másik szobába, és eltöröd a fa darabot. A fa darab mögött találsz egy kis dobozt. Talán egy kulcsal ki lehet nyitni.")
            items.append("doboz")
            rooms.append(room_number)
            if "kulcs" in items:
